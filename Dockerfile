...
# Copy built artifacts: gems, application, bootsnap cache, precompiled assets
COPY --from=build "${BUNDLE_PATH}" "${BUNDLE_PATH}"
COPY --from=build /scrooge /scrooge

# Run and own only the runtime files as a non-root user for security
RUN groupadd --system --gid 1000 scrooge && \
    useradd --uid 1000 --gid 1000 --create-home --shell /bin/bash scrooge && \
    chown -R scrooge:scrooge db log storage tmp
USER scrooge:scrooge

# Entrypoint prepares the database.
ENTRYPOINT ["/scrooge/bin/docker-entrypoint"]
